...
const Category = (props) => {
  ...
  useEffect(() => {
    categoryApis.getAllCategories(auth.userId, auth.token).then((response) => {
  ...
  const addCategoryModal = useSelector(
    (state) => state.toggler.addCategoryModal
  );
  const editCategoryModal = useSelector(
    (state) => state.toggler.editCategoryModal
  );
        ...
        <div className="category-action">
          <div className="category-action-card">
            ...
            <div className="pro-tip">
              <img
                className="pro-tip-img"
                src="https://cdn0.iconfinder.com/data/icons/ui-beast-6/32/UI-19-128.png"
              />
